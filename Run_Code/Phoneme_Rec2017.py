...
def fireLED(channel):
    ...
    FN[0]['recfile'] = FN[0]['recdir'] + "/" + QUES[0]['sessionID'] +"_"+QUES[2]['host_name']+".txt"
    inout = []
    TTL_time = time.time()
    #time.sleep(1)
...
try:
		...
		" -pf high -ex fixedfps -vf -hf -p '0, 0, 960, 540' -ex off -ss "+str(QUES[1]['framedur']))
                ...
                print('Edge detected')
                var_file = open((FN[0]['recdir']+"/"+FN[0]['recdir']+"_var.txt"),'w')
                pickle.dump((QUES,FN,batch_num),var_file)
                var_file.close()
                nbatch += 1
